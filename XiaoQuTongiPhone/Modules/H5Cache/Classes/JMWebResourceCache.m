...
@implementation JMWebResourceCache
...
- (void)commonInit {
    _cachePath = [JMWebCachePaths interceptWebResourceCachePath];
    ...
    if ([fm fileExistsAtPath:_cachePath isDirectory:&isDir]) {
        ...
        if (!isDir) {
            ...
            if ([fm removeItemAtPath:_cachePath error:&error]) { // remove it
                ...
                NSLog(@"remove placeholder file:%@ did failed:%@", _cachePath);
            }
        }
        ...
        succeed = [fm createDirectoryAtPath:_cachePath
                        withIntermediateDirectories:YES
                                         attributes:nil
                                              error:&error];
        if (error != nil) {
            NSLog(@"create cache directory:%@ occur an error:%@", _cachePath);
        }
    }
...
- (NSData *)cacheDataForKey:(NSString *)key url:(NSURL *)url response:(NSURLResponse **)response {
    ...
    if (response != NULL) {
        ...
        if (contentType == nil) {
            ...
            contentType = [JMWebResourceCache mimeTypeBaseOnExtension:extension];
        }
...
- (void)storeCacheData:(NSData *)data
    ...
    [JMUtility performActionOnSubThreadAsync:YES action:^(void) {
        NSString *path = [_cachePath stringByAppendingPathComponent:key];
