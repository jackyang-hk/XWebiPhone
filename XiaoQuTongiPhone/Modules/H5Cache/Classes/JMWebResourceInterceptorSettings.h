//
//  JMWebResourceInterceptorSettings.h
//  JoyMapsKit
//
 ...
 *  Web资源拦截器接口定义
 */
@protocol JMWebResourceInterceptorSettings <NSObject>
 @required
...
/**
 *  全局请求域名白名单
 *  检查当前请求关联的`Host`或`Referer`是否在黑名单内，
 *  如果不在白名单内，不发送网络请求
 */
@property (nonatomic, strong) NSArray *outwhitelist;
 ...
 *  Web资源拦截配置默认实现类。
 */
@interface JMWebResourceInterceptorSettings : TBModel <JMWebResourceInterceptorSettings>
