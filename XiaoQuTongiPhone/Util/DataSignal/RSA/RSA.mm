...
BigInt RSA::encode2(const string &message)
{
	// The new number will be created using a string object (encoded), 
	// and converted into a BigInt on return.
	//string encoded;
	//encoded.resize(message.length() * 2 + 1);
	//unsigned long int index = message.length() * 2;
	BigInt ret((unsigned long)message[0]);
	
	for (int i = 1; i < message.length(); i++)
	{
		ret *= 256;
		ret += (unsigned long)message[i];
	}
	return ret;
}
