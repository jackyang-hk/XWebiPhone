...
void destructiveMulAdd(unsigned char * binary, unsigned int y, unsigned int z) {
	...
	for (int i = 0; i < 512; i++) {
		product = y * (unsigned int)binary[i] + carry;
		binary[i] = product & 0xff;

		carry = (product >> 8) & 0xff;
		//NSLog(@"product %d carry %d", product, carry);		
	}
	
	// Perform the addition
	unsigned long sum = (unsigned int)binary[0] + z;
	binary[0] = (unsigned int)sum;
	carry = sum >> 8;
	for (int i = 1; i < 512; i++) {
		sum = binary[i] + carry;
		//NSLog(@"sum %d carry %d", sum, carry);		
		binary[i] = sum & 0xff;
		carry = (sum >> 8) & 0xff;
...
/* Returns the value of BigInt as std::string. */
std::string BigInt::ToHexString(bool forceSign) const
{
	std::string number1;
	...
	binary[0] = digits[digitCount-1];
	//NSLog(@"binary[0] %d, digit 0 %d",binary[0], digits[0]);
	
	for (int i = digitCount-2 ; i >= 0; i --)
	{
		//NSLog(@"%d", digits[i]);
		destructiveMulAdd(binary, 10, digits[i]);
	}

	
	int offset;
	for (offset = 511; offset > 0 ; offset --)
	{		
		if (binary[offset] != 0)
			break;
	}
	...
	{
		unsigned short b = binary[i];		
			
		unsigned char c = (b>>4) & 0x0f;
		if (c > 9)
		{
			number1.push_back('a'+c - 10);
		}
		else
		{
			number1.push_back('0'+c);
		}			
